...
export const getById = async (DB: D1Database, id: string, fields?: string)
  ...
  const stmt = DB.prepare('SELECT * FROM Units WHERE id=?').bind(id)
...
export const getAll = async (DB: D1Database, userId: string, fields?: string)
  ...
  const resp = await DB.prepare('SELECT * FROM Units WHERE userId=?').bind(userId).all()
...
export const create = async (DB: D1Database, param: any): Promise<Unit | undefined> => {
  ...
  const result: any = await DB.prepare('INSERT INTO Units(id,userId,type,block,floor,number) VALUES(?,?,?,?,?,?)').bind(
    newRec.id,
    newRec.userId,
    newRec.type,
    newRec.block,
...
export const deleteById = async (DB: D1Database, id: string)
  : Promise<boolean> => {
  if (id == null) throw new Error('Missing id!')
  const result: any = await DB.prepare('DELETE FROM Units WHERE id=?').bind(id).run()
  if (!result.success) throw new Error(result)
  return true
}
