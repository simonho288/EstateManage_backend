...
export const updateById = async (env: Env, userId: string, id: string, param: any)
  ...
  if (param.password != null) {
    param.password = await Util.encryptString(param.password, env.USER_ENCRYPTION_KEY, Util.getRandomInt(10001, 99999))
  }
...
export const updateProperty = async (env: Env, userId: string, field: string, value: any): Promise<boolean> => {
  ...
  if (field === 'password') {
    value = await Util.encryptString(value, env.USER_ENCRYPTION_KEY, Util.getRandomInt(10001, 99999))
    // } else if (field === 'email') {
