...
export const getById = async (DB: D1Database, id: string, fields?: string)
  ...
  const stmt = DB.prepare('SELECT * FROM Folders WHERE id=?').bind(id)
...
export const getAll = async (DB: D1Database, userId: string, fields?: string)
  ...
  const resp = await DB.prepare('SELECT * FROM Folders WHERE userId=?').bind(userId).all()
...
export const create = async (DB: D1Database, param: any): Promise<Folder | undefined> => {
  ...
  const result: any = await DB.prepare('INSERT INTO Folders(id,userId,dateCreated,name,isPublic,status) VALUES(?,?,?,?,?,?)').bind(
    newRec.id,
    newRec.userId,
    newRec.dateCreated,
    newRec.name,
...
export const deleteById = async (DB: D1Database, id: string)
  : Promise<boolean> => {
  if (id == null) throw new Error('Missing id!')
  const result: any = await DB.prepare('DELETE FROM Folders WHERE id=?').bind(id).run()
  if (!result.success) throw new Error(result)
  return true
}
