...
const DATALIST_NAME = 'residencesDataList'
const DATAFORM_NAME = 'residenceDataForm'
    ...
    <div class="ui middle aligned three column centered grid">
      <div class="row">
        <div class="left floated eight wide column">
          <label class="ui huge header">Residences</label>
        </div>
...
export class Residences implements IPage {
  _recId?: string
  ...
  public async openPage() {
    console.log('Residences::openPage()')

    globalThis.app.setContentMarkup(CONTENT_MKUP)
  }

  public async loadData() {
    console.log('Residences::loadData()')
    ...
    if (this._datalistData == null) {
      ...
      this._datalistData = Util.sortUnits('residences', resp.data)
    }
  ...
  private async onAddNew(evt: Event) {
    console.log('Residences::onAddNew()')
    evt.preventDefault()
    ...
    <div class="ui container dataEditForm">
      <h2 class="title">Add New Residence</h2>
      ${editAutoformMkup}
    </div>
    `
  ...
  private async onEdit(id: string) {
    console.log(`Residences::onEdit('${id}')`)
    ...
    let html = `
    <div class="ui container dataEditForm">
      <h2 class="title">Edit Residence Record</h2>
      ${editAutoformMkup}
    </div>
    `
    globalThis.app.setContentMarkup(html)
  ...
  private buildAutoformHtml(mode: FormMode, record: object) {
    ...
    this._autoform = new AutoForm({
      ...
      fields: [{
        ...
        label: 'Room',
        isRequired: true,
        placeholder: 'Room number or name e.g. A or 1',
      }],
  ...
  private async getAdditonalCheckingError(submittedValues: any): Promise<string | null> {
    ...
    if (this._autoform.mode === FormMode.New) {
      let result = await Ajax.getRecsByCrit('units', {
        crit: `block='${block}' AND floor='${floor}' AND number='${number}' AND type='res'`,
        fields: 'id',
      })
      if (result.data.length > 0) return `Same residence is exist`
    }
  ...
  private async onFormSubmitted(evt: Event) {
    console.log('Residences::onFormSubmitted()')
